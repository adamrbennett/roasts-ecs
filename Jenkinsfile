node {
  ...
  stage('Deploy') {
    ...
    sh "aws elbv2 create-rule --listener-arn ${ALB_LISTENER_ARN} --conditions Field=host-header,Values=roasts-${env.BUILD_NUMBER}.${DOMAIN} --priority 100 --actions Type=forward,TargetGroupArn=${target_group_arn}"
  }
}
